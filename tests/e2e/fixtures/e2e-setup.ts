...
/**
 * Combined test fixture that includes:
 * - Runway API mocks (to avoid real API calls)
 * - Database mocks (to return test data)
...
export const testFailure = base.extend({
  context: async ({ context }, useFixture) => {
    ...
    // === DATABASE MOCKS (same as success case) ===
    await context.route('**/rest/v1/projects**', async route => {
      await route.fulfill({
        status: 200,
        headers: { 'content-type': 'application/json' },
    ...
    // For failure tests, don't create videos even after POST
    await context.route('**/rest/v1/generated_videos**', async route => {
      await route.fulfill({
        status: 200,
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify([]), // Always return empty for failure tests
      });
    });
